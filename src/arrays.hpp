...
#ifndef QLALIB_ARRAYS
#define QLALIB_ARRAYS
...
{
protected:
  static unsigned int memSize;  ///< Total number of bytes in memory storing all array data structures
  size_t nx;                    ///< Number of array elements
  T xinit;                      ///< Value of first element, useful for various function
  T xstep;                      ///< \f$dx\f$ used in integration and differentiation routines
  std::unique_ptr<T[]> vals;    ///< Data array
...
public:
  ...
  Array1D(const int NX) : nx(NX), xinit(0.0), xstep(0.0), vals(std::unique_ptr<T[]>(new T[NX]))
  {
    zero();
    memSize += sizeof(T)*size(); //number of bytes allocated to instance of class
  }
...
//  Access functions
  ...
  /**
  *   \brief Fills the entire array. The \f$ii^{th}\f$ element is assigned \f$ xinit+ii*xstep \f$.
  **/
  void fill_array()
  {
  ...
  /**
   * @brief Returns reference to the values in the array
   *
   * @param nn Element index
   * @return Reference to array at position
   */
  ...
  /**
   * @brief Scale every member of the array by a multiplicative constant
   *
   * @param a Constant of any type (int,double,float...)
   * @tparam U Variable type
   */
